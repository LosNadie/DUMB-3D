// Mock data with real iTunes album covers
const now = new Date()
...
export default [
  { url:'/api/reviews',method:'get',response:()=>({code:0,message:'ok',data:musicReviews}) },
  { url:/\/api\/reviews\/(\d+)/,method:'get',response:(_:any,path:string)=>{const id=Number(path.match(/\/api\/reviews\/(\d+)/)?.[1]);return{code:0,message:'ok',data:musicReviews.find((r:any)=>r.id===id)||musicReviews[0]}} },
  { url:'/api/anime',method:'get',response:()=>({code:0,message:'ok',data:animes}) },
  { url:/\/api\/anime\/(\d+)/,method:'get',response:(_:any,path:string)=>{const id=Number(path.match(/\/api\/anime\/(\d+)/)?.[1]);return{code:0,message:'ok',data:animes.find((a:any)=>a.id===id)||animes[0]}} },
  { url:'/api/movie',method:'get',response:()=>({code:0,message:'ok',data:movies}) },
  { url:/\/api\/movie\/(\d+)/,method:'get',response:(_:any,path:string)=>{const id=Number(path.match(/\/api\/movie\/(\d+)/)?.[1]);return{code:0,message:'ok',data:movies.find((m:any)=>m.id===id)||movies[0]}} },
  { url:'/api/admin/reviews',method:'get',response:()=>({code:0,message:'ok',data:musicReviews}) },
  { url:'/api/admin/anime',method:'get',response:()=>({code:0,message:'ok',data:animes}) },
  { url:'/api/admin/movie',method:'get',response:()=>({code:0,message:'ok',data:movies}) },
  { url:/\/api\/admin\/reviews\/(\d+)/,method:'get',response:(_:any,path:string)=>{const id=Number(path.match(/\/api\/admin\/reviews\/(\d+)/)?.[1]);return{code:0,message:'ok',data:musicReviews.find((r:any)=>r.id===id)||musicReviews[0]}} },
  { url:/\/api\/admin\/anime\/(\d+)/,method:'get',response:(_:any,path:string)=>{const id=Number(path.match(/\/api\/admin\/anime\/(\d+)/)?.[1]);return{code:0,message:'ok',data:animes.find((a:any)=>a.id===id)||animes[0]}} },
  { url:/\/api\/admin\/movie\/(\d+)/,method:'get',response:(_:any,path:string)=>{const id=Number(path.match(/\/api\/admin\/movie\/(\d+)/)?.[1]);return{code:0,message:'ok',data:movies.find((m:any)=>m.id===id)||movies[0]}} },
  { url:'/api/auth/login',method:'post',response:()=>({code:0,message:'ok',data:{token:'mock-token',username:'admin',role:'admin'}}) },
  { url:'/api/auth/register',method:'post',response:()=>({code:0,message:'ok',data:{token:'mock-token',username:'user',role:'user'}}) },
  { url:'/api/comments',method:'get',response:()=>({code:0,message:'ok',data:[{id:1,contentType:'REVIEW',contentId:1,userId:1,username:'乐迷',content:'写得真好！',score:9,createTime:ago(0,1)}]}) },
]
