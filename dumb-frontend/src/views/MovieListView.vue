...
function resolveCoverUrl(url?: string) {
  if (!url) return 'https://dummyimage.com/400x400/111/eee&text=MOVIE'
  ...
  <section class="wax-store" :class="{ 'cube-intro': introFromCube }">
    ...
    <section class="wax-filter">
      ...
      <div class="filter-group">
        ...
        <el-select v-model="filters.region" clearable placeholder="ALL">
      ...
      >
        ...
        >
          <div class="poster-hover">
            ...
            <p class="poster-meta">{{ item.director || '-' }}</p>
            <p class="poster-meta">{{ (item.genre || '').split(',').map((g: string) => g.trim()).filter(Boolean)[0] || '-' }}</p>
            <p class="poster-meta">{{ item.region || '-' }} · {{ item.releaseDate?.split('-')[0] || '-' }}</p>
          </div>
        </div>
      </div>
    </div>
